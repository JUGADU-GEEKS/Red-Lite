...
const LaneUpload = ({ 
  ...
  return (
    ...
    >
      <motion.div
        className={`relative w-96 h-72 bg-gradient-to-br from-gray-800/60 to-gray-900/60 backdrop-blur-sm border-2 ${
      ...
      >
        ...
        <div className="flex items-center justify-center mb-4">
          <motion.div
            className={`w-12 h-12 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white font-bold text-xl ${directions[direction].rotation}`}
          ...
          <div className="ml-3">
            <h3 className="text-lg font-semibold text-white">{directions[direction].label}</h3>
            <div className="flex items-center gap-2">
              <Camera className="w-4 h-4 text-blue-400" />
              <span className="text-sm text-gray-400">Lane {position + 1}</span>
            </div>
          </div>
        </div>
        ...
        {!frame ? (
          <motion.div
            className="relative border-2 border-dashed border-gray-600 rounded-xl p-6 text-center h-40 flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 transition-colors"
          ...
          >
            ...
            <div className="absolute top-2 right-2">
              <motion.div
                className="bg-green-500/80 backdrop-blur-sm rounded-full p-2"
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(counts).map(([vehicle, count]) => {
                ...
                return (
                  <motion.div
                    key={vehicle}
                    className="bg-gray-800/60 rounded-lg p-2 flex items-center justify-between"
...
function Detection() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white overflow-x-hidden relative">
      ...
      >
        <motion.div
          className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600/20 to-purple-600/20 backdrop-blur-sm border border-blue-500/20 rounded-full px-6 py-2 mb-4"
          whileHover={{ scale: 1.05 }}
        >
          <Camera className="w-5 h-5 text-blue-400" />
          <span className="text-sm font-medium text-blue-300">Live Traffic Detection</span>
        </motion.div>
      ...
      {/* Stats Bar */}
      <motion.div
        className="relative z-10 max-w-6xl mx-auto px-6 mb-8"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8, delay: 0.2 }}
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-gray-800/60 to-gray-900/60 backdrop-blur-sm border border-gray-700/50 rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-blue-400 mb-1">{getActiveLanes()}/4</div>
            <div className="text-sm text-gray-400">Active Lanes</div>
          </div>
          {Object.entries(getTotalCounts()).slice(0, 3).map(([vehicle, count]) => (
            <div key={vehicle} className="bg-gradient-to-br from-gray-800/60 to-gray-900/60 backdrop-blur-sm border border-gray-700/50 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-emerald-400 mb-1">{count}</div>
              <div className="text-sm text-gray-400 capitalize">{vehicle}s</div>
            </div>
          ))}
        </div>
      </motion.div>
      ...
      {/* Crossroad Layout */}
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[800px] px-6 gap-y-20">
        {/* Row 1: North & East */}
        <div className="flex flex-row justify-between items-center w-full max-w-5xl gap-x-20">
      ...
      {/* Overall Statistics */}
      {Object.keys(getTotalCounts()).length > 0 && (
        <motion.div
          ...
          transition={{ duration: 0.8 }}
        >
          <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 backdrop-blur-sm border border-blue-500/20 rounded-3xl p-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {Object.entries(getTotalCounts()).map(([vehicle, count]) => {
                ...
                return (
                  <motion.div
                    key={vehicle}
                    className="text-center p-4 bg-gray-800/60 rounded-xl"
                    whileHover={{ scale: 1.05, backgroundColor: "rgba(55, 65, 81, 0.8)" }}
                  >
                    <VehicleIcon className="w-8 h-8 text-blue-400 mx-auto mb-3" />
                    <div className="text-2xl font-bold text-emerald-400 mb-1">{count}</div>
                    <div className="text-sm text-gray-300 capitalize">{vehicle}s Detected</div>
                  </motion.div>
                );
              })}
            </div>
